  ...
  def input_fn():
    ...
    with tf.name_scope("input_fn"):
      for problem_idx in xrange(problem_count):
        ...
        feature_map = features_for_problem(
            ...
            shard=shard,
            name="problem_%d" % problem_idx)   # feature_map is inputs, targets, space_id
        problem_batches.append(feature_map)  # add batch inputs and targets
    ...
    if fixed_problem is None:
      ...
      feature_map = cond_on_index(
          ...
          problem_count - 1)  # one specific feature_map for one problem
    else:
    ...
    feature_map["problem_choice"].set_shape([])  # one scalar
    feature_map["target_space_id"].set_shape([])  # one scalar
