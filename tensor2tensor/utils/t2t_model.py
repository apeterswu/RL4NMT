...
class T2TModel(object):
  ...
  def __init__(self,
    ...
    if hparams.shared_embedding_and_softmax_weights:
      ...
      for problem in hparams.problems:
        if "inputs" in problem.input_modality:  # modality was created when building dataset
  ...
  def _create_modalities(self, problem_hparams, hparams):
    ...
    for f, modality_spec in six.iteritems(problem_hparams.input_modality):
      ...
      # initialize and create modality: call symbol modality in modalities.
  ...
  def ffn_layer_for_baseline(self, body_output, hparams):
    """Feed-forward layer for baseline in the REINFORCE-based transformer.
    ...
    """
    body_output = tf.squeeze(body_output, 0)   # add one squeeze to remove non useful dimension
    conv_output = common_layers.conv_hidden_relu(
      body_output,
      hparams.filter_size,
      1,
      dropout=hparams.relu_dropout)  # two layers transformation. hidden_size->filter_size->1
    return conv_output

  def baseline_loss(self, predict_baseline, value):
      return tf.reduce_sum((predict_baseline - value)**2)

  def model_fn(self, features, skip=False, force_full_predict=False, mrt=False):
    ...
    # assume shared_features is same as feature, since we only use one datashard
    sharded_features = self._shard_features(features)
    # shard_features will also split samples
    sharded_samples = sharded_features.get("samples", None)  # indicate rl training with samples
    sharded_values = sharded_features.get("values", None)
    ...
    for key, input_modality in six.iteritems(
      ...
      if sharded_samples:
        do_reuse = True
      transformed_features[key + "_raw"] = sharded_features[key]  # original word ids
      with tf.variable_scope(input_modality.name, reuse=do_reuse):
        transformed_features[key] = input_modality.bottom_sharded(
            sharded_features[key], dp)   # feed in features[inputs], return batch input embedding
    ...
    with tf.variable_scope(target_modality.name, reuse=target_reuse):
        if sharded_samples:
            transformed_features["targets"] = target_modality.targets_bottom_sharded(
                sharded_samples, dp)   # return sampled sentence embedding
        else:
            transformed_features["targets"] = target_modality.targets_bottom_sharded(
              sharded_features["targets"], dp)   # return batch targets embedding

    # Allows later access to pre-embedding raw targets.
    transformed_features["targets_raw"] = sharded_features["targets"]  # original word ids

    # Construct the model body.
    to_resue = False
    if self._problem_idx > 0 or sharded_samples:
      to_resue = True
    with tf.variable_scope("body", reuse=to_resue):
      if skip:
        body_outputs = transformed_features["targets"]
        losses = {"extra": 0.0}
      else:
        body_outputs, losses = self.model_fn_body_sharded(
            transformed_features)    # list of tensor, each tensor is [batch, timesteps, dim]
        # model_fn_body will search transformed_features["targets"], transformed_features include inputs and targets
        # transformed_features: inputs embedding, targets embedding; loss = {"extra": 0.0}
        if not isinstance(losses, dict):  # If it's a single extra loss.
          losses = {"extra": losses}
    if sharded_samples:
      use_baseline = getattr(self._hparams, 'use_baseline', False)
      if use_baseline:
        print("###Use baseline in REINFORCE training.###")
        with tf.variable_scope("body_baseline", reuse=tf.AUTO_REUSE):
          sharded_body_outputs = tf.stop_gradient(body_outputs)   # here for body_outputs, stop compute gradients.
          sharded_baseline = dp(self.ffn_layer_for_baseline, sharded_body_outputs, self._hparams)
          sharded_baseline_loss = dp(self.baseline_loss, sharded_baseline, sharded_values)
          training_baseline_loss = tf.add_n(sharded_baseline_loss)
          losses["training_baseline"] = training_baseline_loss
    with tf.variable_scope(target_modality.name, reuse=target_reuse):
      last_only = (target_modality.top_is_pointwise and
                   self._hparams.mode == tf.estimator.ModeKeys.PREDICT and
                   not force_full_predict)
      if not last_only:  # training or eval mode
          """Actually, samples or shared_features["targets"] are not used in top_sharded"""
          if sharded_samples:
            sharded_logits = target_modality.top_sharded(
                body_outputs, sharded_samples, dp)   # return output logits
            sharded_loss_num, sharded_loss_den = target_modality.loss_sharded(
                sharded_logits, sharded_samples, dp)  # return sampled words log probs
            # training_loss is [batch * timesteps] for rl training
            for i in range(len(sharded_loss_num)):
                if mrt:   # build the mrt loss
                    tmp = tf.reduce_sum(sharded_loss_num[i], axis=1)  # total loss
                    tmp *= 0.0005
                    tmp -= tmp.min()
                    tmp = tf.exp(-tmp)
                    tmp /= tf.reduce_sum(tmp)
                    tmp *= tf.reduce_sum(sharded_values[i], axis=1)   # sentence reward
                    tmp -= tf.reduce_sum(tmp)
                    sharded_loss_num[i] = tmp
                    sharded_loss_num[i] = tf.reduce_sum(sharded_loss_num[i])
                else:
                    sharded_loss_num[i] = tf.reduce_sum(sharded_loss_num[i] * sharded_values[i])
                sharded_loss_den[i] = tf.reduce_sum(sharded_loss_den[i])
          else:
            sharded_logits = target_modality.top_sharded(   # "targets" not used here
                body_outputs, sharded_features["targets"], dp)   # calculate the output logits
            sharded_loss_num, sharded_loss_den = target_modality.loss_sharded(  # "targets" are used for loss
                sharded_logits, sharded_features["targets"], dp)  # calculate the loss
            """Regular MLE training, training loss is a summed scalar."""
            for i in range(len(sharded_loss_num)):
                sharded_loss_num[i] = tf.reduce_sum(sharded_loss_num[i])
                sharded_loss_den[i] = tf.reduce_sum(sharded_loss_den[i])
          training_loss = tf.add_n(sharded_loss_num) / tf.maximum(1.0, tf.add_n(sharded_loss_den))
          # training_loss = tf.reduce_sum(training_loss)
    ...
    """For rl training, combine mle loss together training."""
    combine_mle = getattr(self._hparams, "combine_mle", False)
    if combine_mle:
        print("##Training combine with mle##")
        with tf.variable_scope(target_modality.name, reuse=target_reuse):
            transformed_features["targets"] = target_modality.targets_bottom_sharded(
                sharded_features["targets"], dp)  # return batch targets embedding
        with tf.variable_scope("body", reuse=to_resue):
            body_outputs, _ = self.model_fn_body_sharded(
                transformed_features)  # list of tensor, each tensor is [batch, timesteps, dim]
            # model_fn_body will search transformed_features["targets"], transformed_features include inputs and targets
            # transformed_features: inputs embedding, targets embedding; loss = {"extra": 0.0}
        with tf.variable_scope(target_modality.name, reuse=target_reuse):
            """Actually, samples or shared_features["targets"] are not used in top_sharded"""
            sharded_target_logits = target_modality.top_sharded(  # "targets" not used here
                body_outputs, sharded_features["targets"], dp)  # calculate the output logits
            sharded_target_loss_num, sharded_target_loss_den = target_modality.loss_sharded(
                sharded_target_logits, sharded_features["targets"], dp)  # calculate the loss
            """Regular MLE training, training loss is a summed scalar."""
            for i in range(len(sharded_target_loss_num)):
                sharded_target_loss_num[i] = tf.reduce_sum(sharded_target_loss_num[i])
                sharded_target_loss_den[i] = tf.reduce_sum(sharded_target_loss_den[i])
            mle_training_loss = tf.add_n(sharded_target_loss_num) / tf.maximum(1.0, tf.add_n(sharded_target_loss_den))
            mle_training_loss *= self._problem_hparams.loss_multiplier
        losses["mle_training"] = mle_training_loss
    ...
    if do_scheduled_sampling:
      ...
      def sampled_results():
        ...
        sampled_targets = dp(sample, sharded_logits)   # sampled_targets: word ids
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=True):
          with tf.variable_scope(target_modality.name):
            new_features["targets"] = target_modality.targets_bottom_sharded(
                new_targets, dp)   # new targets embedding including samples
          with tf.variable_scope("body"):
            body_outputs, losses = self.model_fn_body_sharded(new_features)  # calculate body outputs before logits
            if not isinstance(losses, dict):  # If it's a single extra loss.
              losses = {"extra": losses}
          with tf.variable_scope(target_modality.name):
            new_sharded_logits = target_modality.top_sharded(
                body_outputs, sharded_features["targets"], dp)   # output targets are still true targets
            sharded_loss_num, sharded_loss_den = target_modality.loss_sharded(
                sharded_logits, sharded_features["targets"], dp)  # calculate true targets loss
            """Regular MLE training, training_loss is a summed scalar."""
            for i in range(len(sharded_loss_num)):
                sharded_loss_num[i] = tf.reduce_sum(sharded_loss_num[i])
                sharded_loss_den[i] = tf.reduce_sum(sharded_loss_den[i])
            training_loss = tf.add_n(sharded_loss_num) / tf.maximum(1.0, tf.add_n(sharded_loss_den))
    ...
    # losses["training"] are matrix for rl training
