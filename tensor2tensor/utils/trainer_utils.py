...
def create_experiment_components(data_dir, model_name, hparams, run_config):
  ...
  eval_input_fn = input_fn_builder.build_input_fn(
      ...
      worker_replicas=FLAGS.worker_replicas,
      worker_id=FLAGS.worker_id,
      dataset_split="test" if FLAGS.eval_use_test_set else None)  # evaluate on test dataset
...
def add_problem_hparams(hparams, problems):
  ...
  for problem_name in problems.split("-"):
    try:
      problem = registry.problem(problem_name)   # search for the registered problem
    except LookupError:
      all_problem_names = sorted(registry.list_problems())  # list all problems
...
def create_run_config(output_dir):
  ...
  run_config = tf.contrib.learn.RunConfig(
      ...
      log_step_count_steps=10,   # monitor the training status for each 10 steps
