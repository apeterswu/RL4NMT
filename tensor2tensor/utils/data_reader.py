...
def input_pipeline(problem,
  ...
  with tf.name_scope("input_pipeline"):
    dataset = problem.dataset(
        ...
        shard=shard)  # feature_map in the dataset; when call problem.dataset, it will create input/target modalities
    ...
    skip_count = getattr(hparams, 'skip_count', None)    # skip the several batch data only if set the 'skip_count'
    if skip_count is not None:
      print("Dataset will skip {} batch data.".format(skip_count))
    ...
    batched_examples = dataset.make_one_shot_iterator().get_next()   # get one iterator batch data
