  ...
  * `loss` would compare the generated image to the target image and score it.
  ...
  def loss(self, top_out, targets, weights_fn=common_layers.weights_nonzero):
    ...
    return common_layers.padded_cross_entropy(
        ...
        reduce_sum=False,  # return tensor [batch * timesteps] for rl training
        use_focal_loss=self._model_hparams.use_focal_loss,
        focal_loss_gamma=self._model_hparams.focal_loss_gamma)
  ...
  def loss_sharded(self, sharded_top_out, sharded_targets, data_parallelism):
    ...
    # sharded_loss_num and sharded_loss_den are list
    # loss = tf.add_n(sharded_loss_num) / tf.maximum(1.0,
    #                                                tf.add_n(sharded_loss_den))
    # loss should be the same shape as sharded_loss_num
    return sharded_loss_num, sharded_loss_den
