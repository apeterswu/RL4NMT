...
def make_input_fn_from_generator(gen):
  ...
  first_ex = six.next(gen)  # get one batch data_ids (dictionary)
...
def _decode_batch_input_fn(problem_id, num_decode_batches, sorted_inputs,
  ...
  for b in range(num_decode_batches):
    ...
    for inputs in sorted_inputs[b * batch_size:(b + 1) * batch_size]:
      input_ids = vocabulary.encode(inputs)   # word to id
      if max_input_size > 0:
    ...
    for input_ids in batch_inputs:
      ...
      x = input_ids + [0] * (batch_length - len(input_ids))  # append 0, to be same length as batch_length
...
def _decode_input_tensor_to_features_dict(feature_map, hparams):
  ...
  input_space_id, target_space_id, x = input_fn_builder.cond_on_index(
      input_fn, feature_map["problem_choice"], len(hparams.problems) - 1)   # x is input tensor
  ...
  features["target_space_id"] = target_space_id
  features["decode_length"] = (
      IMAGE_DECODE_LENGTH if input_is_image else tf.shape(x)[1] + 50)  # input_length + 50
  features["inputs"] = x
  return features
