...
class Transformer(t2t_model.T2TModel):
  ...
  def model_fn_body(self, features):
    ...
    inputs = features.get("inputs")  # inputs embedding
    ...
    targets = features["targets"]  # inputs embedding
...
def transformer_ffn_layer(x, hparams, pad_remover=None):
  ...
  if hparams.ffn_layer == "conv_hidden_relu":
    ...
    conv_output = common_layers.conv_hidden_relu(
        ...
        hparams.filter_size,
        hparams.hidden_size,
        dropout=hparams.relu_dropout)  # two layers transformation. hidden_size->filter_size->hidden_size
    if pad_remover:
...
def transformer_base_v1():
  ...
  hparams.add_hparam("use_focal_loss", int(False)) 
  hparams.add_hparam("focal_loss_gamma", 0.0)    
