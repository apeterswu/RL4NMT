  ...
  if hparams.prepend_mode != "none":
    if mode == tf.estimator.ModeKeys.PREDICT:
      ...
      example["targets"] = tf.concat(
          [example["inputs"], [0], example["targets"]], 0)  # concat inputs and targets with [0]
  return example
...
class Problem(object):
  ...
  def get_hparams(self, model_hparams=None):
    ...
    hp = _default_hparams()    # empty
    ret = self.hparams(hp, model_hparams)  # define inputs/targets modality
    if ret is not None:
      raise ValueError("The Problem subclass hparams function should mutate "
                       "the defaults passed in and return None.")

    hp.add_hparam("vocabulary", self._encoders)   # add vocabulary
    ...
    self._hparams = hp
    return self._hparams   # including inputs/targets modality
  ...
  def dataset(self,
    ...
    _ = self.get_hparams(hparams)   # call self.hparams(), which will define vocabulary and others empty (modality)

    data_fields, data_items_to_decoders = self.example_reading_spec()
    if data_items_to_decoders is None:  # None
      data_items_to_decoders = {    # data_fields: {"inputs", "targets"}, define tensor type
    ...
    # training data filepattern: xxx-train-00000-of-00001
    tf.logging.info("Reading data files from %s", data_filepattern)
    data_files = tf.contrib.slim.parallel_reader.get_data_files(
        data_filepattern)
    if shuffle_files or shuffle_files is None and is_training:
      random.shuffle(data_files)   # shuffle data
    dataset = tf.contrib.data.TFRecordDataset(data_files)  # Dataset build
    ...
    def _preprocess(example):
      example = self.preprocess_example(example, mode, hparams)  # process input sentence
    ...
    return dataset   # feature_map in the dataset
...
class Text2TextProblem(Problem):
  ...
  def generate_data(self, data_dir, tmp_dir, task_id=-1):
    train_paths = self.training_filepaths(
        data_dir, self.num_shards, shuffled=False)  # problem_name-train-00000-of-00001
    dev_paths = self.dev_filepaths(
        data_dir, self.num_dev_shards, shuffled=False)  # problem_name-dev-00000-of-00001
    ...
    # self.generator() return source, target ids
