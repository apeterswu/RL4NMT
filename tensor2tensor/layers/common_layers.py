...
def conv_hidden_relu(inputs,
  ...
  with tf.variable_scope(name, "conv_hidden_relu", [inputs]):
    ...
    h = conv_f1(
        ...
        activation=tf.nn.relu,
        name="conv1",
        **kwargs)   # hidden_size = 4096; output_size = 2048; 2048->4096
    if dropout != 0.0:
      h = tf.nn.dropout(h, 1.0 - dropout)
    conv_f2 = conv if second_kernel_size == (1, 1) else separable_conv
    ret = conv_f2(h, output_size, second_kernel_size, name="conv2", **kwargs)  # 4096->2048
    if is_3d:
...
def padded_cross_entropy(logits,
  ...
  if isinstance(logits, FactoredTensor):
    return padded_cross_entropy_factored(
        ...
        reduce_sum=reduce_sum)   # one-hot cross-entropy loss
  confidence = 1.0 - label_smoothing
  vocab_size = tf.shape(logits)[-1]
  with tf.name_scope("padded_cross_entropy", [logits, labels]):
    pad_logits, pad_labels = pad_with_zeros(logits, labels)
    # pad_labels: [0, 1] tensor
    xent, hinge_mask = smoothing_cross_entropy(  # label smooth loss
        pad_logits, pad_labels, vocab_size,
        confidence, use_focal_loss=use_focal_loss, focal_loss_gamma=focal_loss_gamma)
    weights = weights_fn(pad_labels)
    if hinge_mask is not None:
      weights = tf.to_float(tf.where(tf.logical_and(
          tf.equal(weights, 1.0), tf.equal(hinge_mask, 1.0)),
          tf.ones_like(weights), tf.zeros_like(weights)))
    if not reduce_sum:
      return xent * weights, weights    # [batch, timesteps]
    return tf.reduce_sum(xent * weights), tf.reduce_sum(weights)  # scalar
  ...
  with tf.name_scope("smoothing_cross_entropy", [logits, labels]):
    ...
    if use_focal_loss:  
      ...
      hard_targets = tf.one_hot(
          ...
          on_value=1.0,
          off_value=0.0)
      ref_probs = tf.reduce_sum(hard_targets * probs, axis=-1)
      if focal_loss_gamma == -1.0:
        tf.logging.info('use_hinge_loss enter')
        hinge_mask = tf.to_float(tf.where(ref_probs>0.8, tf.zeros_like(ref_probs), tf.ones_like(ref_probs)))
        xentropy = -tf.log(ref_probs + 1e-20) * hinge_mask
        return xentropy, hinge_mask
      else:  # focal loss
        tf.logging.info('use_focal_loss enter')
        xentropy = - tf.pow(1. - ref_probs, focal_loss_gamma) * tf.log(ref_probs + 1e-20)  
        return xentropy, None
    else:
      xentropy = tf.nn.softmax_cross_entropy_with_logits(
        logits=logits, labels=soft_targets)
    return xentropy - normalizing, None
...
def padded_cross_entropy_factored(factored_logits,
  ...
  with tf.name_scope("padded_cross_entropy_factored", [a, b, labels]):
    ...
    weights = weights_fn(labels)  # [0,1] matrix of target tokens
    if not reduce_sum:
