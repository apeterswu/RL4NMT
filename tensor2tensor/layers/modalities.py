...
class SymbolModality(modality.Modality):
  ...
  def bottom_simple(self, x, name, reuse):
    with tf.variable_scope(name, reuse=reuse):
      ...
      var = self._get_weights()  # get embedding matrix
      ret = tf.gather(var, x)  # get var according to x
      if self._model_hparams.multiply_embedding_mode == "sqrt_depth":
        ret *= self._body_input_depth**0.5
      ret *= tf.expand_dims(tf.to_float(tf.not_equal(x, 0)), -1)  # embedding
      return ret

  def bottom(self, x):
    self._bottom_was_called = True
    if self._model_hparams.shared_embedding_and_softmax_weights:
      return self.bottom_simple(x, "shared", reuse=None)   # inputs embedding will be shared
    else:
  ...
  def targets_bottom(self, x):
    if self._model_hparams.shared_embedding_and_softmax_weights:
      try:
        return self.bottom_simple(x, "shared", reuse=True)   # targets embedding will be shared
      except ValueError:
  ...
  def top(self, body_output, _):
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
      ...
      var = self._get_weights(body_output_shape[-1])  # vocab*dim: output embedding matrix
